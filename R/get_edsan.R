...
.edsan_parse_fields <- function(fields) {
  if (is.null(fields) || length(fields) == 0 || all(is.na(fields))) return(character())

  x <- as.character(fields)
...
.edsan_call <- function(module, query, what = c("data", "idtriplets"), fields = NULL, ...) {
  ...
  requested_fields <- .edsan_parse_fields(fields)
  core_triplets <- c("ELTID", "EVTID", "PATID")
  extra_fields <- setdiff(requested_fields, core_triplets)
  ...
  if (length(requested_fields) > 0 && "fields" %in% names(formals(fn))) {
    args$fields <- paste(requested_fields, collapse = ",")
  }

  res <- try(do.call(fn, args), silent = TRUE)

  if (inherits(res, "try-error")) {
    return(list(ok = FALSE, value = NULL, error = as.character(res)))
  }
...
.edsan_combine <- function(module, results, what = c("data", "idtriplets"), fields = NULL, ...) {
  what <- match.arg(what)

  requested_fields <- .edsan_parse_fields(fields)
  if (what == "idtriplets") {
    requested_fields <- unique(c("ELTID", "EVTID", "PATID", requested_fields))
  }

  select_requested <- function(df) {
    if (!is.data.frame(df)) return(df)
    if (length(requested_fields) == 0) return(df)

    keep <- intersect(requested_fields, names(df))
    df[, keep, drop = FALSE]
  }
  ...
  if (what == "idtriplets") {
    ...
    if (length(rows) == 0) {
      empty_cols <- requested_fields
      if (length(empty_cols) == 0) empty_cols <- c("ELTID", "EVTID", "PATID")
      out <- stats::setNames(vector("list", length(empty_cols)), empty_cols)
      out[] <- list(character())
  ...
  if (module == "doceds") {
    ...
    if (dropped > 0) {
      warning("Skipped ", dropped, " doceds batch result(s) that were not data frames.")
    }

    out <- dplyr::bind_rows(purrr::compact(coerced))
    out <- select_requested(out)
...
get_edsan <- function(
  ...
  for (chunk_idx in seq_along(id_chunks)) {
    ...
    if (isTRUE(batch_on_error_only)) {
      tr0 <- .edsan_call(module, q, what, fields = fields)
    ...
    results[[length(results) + 1]] <- .edsan_combine(module, tb$results, what, fields = fields)
